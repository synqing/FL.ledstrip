...
uint8_t M5ROTATE8::getAddress() { return _address; }

uint8_t M5ROTATE8::getVersion() { return read8(M5ROTATE8_REG_VERSION); }
...
bool M5ROTATE8::allOff() { return setAll(0, 0, 0); }
...
uint8_t M5ROTATE8::getButtonChangeMask() {
  return read8(M5ROTATE8_REG_BUTTON_MASK) ^ 0xFF; // invert for logical sense
}
...
uint8_t M5ROTATE8::read8(uint8_t reg) {
  ...
  if (_wire->requestFrom(_address, (uint8_t)1) != 1) return 0;
  return _wire->read();
}
...
uint32_t M5ROTATE8::read24(uint8_t reg) {
  ...
  _error = _wire->endTransmission();
  if (_error != 0) return 0;
  if (_wire->requestFrom(_address, (uint8_t)3) != 3) return 0;
  uint32_t value = 0;
  value += _wire->read();
  value <<= 8;
  value += _wire->read();
...
bool M5ROTATE8::write32(uint8_t reg, uint32_t value) {
  _wire->beginTransmission(_address);
  _wire->write(reg);
  _wire->write((uint8_t)(value & 0xFF)); value >>= 8;
  _wire->write((uint8_t)(value & 0xFF)); value >>= 8;
  _wire->write((uint8_t)(value & 0xFF)); value >>= 8;
...
uint32_t M5ROTATE8::read32(uint8_t reg) {
  ...
  _error = _wire->endTransmission();
  if (_error != 0) return 0;
  if (_wire->requestFrom(_address, (uint8_t)4) != 4) return 0;
  uint32_t value = 0;
  value += (_wire->read());
